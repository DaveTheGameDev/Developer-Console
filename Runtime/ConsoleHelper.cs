...
namespace Debugging.DeveloperConsole
{
	public static class ConsoleHelper
	{
			...
			foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
			{
				foreach (var type in assembly.GetTypes())
				{
					foreach (var method in type.GetMethods(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic))
					{
						...
						var command = method.GetCustomAttribute<ConsoleCommand>();
						var commandDesc = method.GetCustomAttribute<CommandDescription>();
		...
		{
			return new CommandData(aliases, description, paramList, method);
		}
